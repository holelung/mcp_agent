...
<template>
  <div class="space-y-8 animate-fade-in">
    ...
    <div class="flex items-start justify-between">
      <div>
        <p class="text-gray-500 font-medium">{{ getGreeting() }}</p>
        <h2 class="text-3xl font-display font-bold text-gray-900 mt-1">대시보드</h2>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
      <div 
        ...
        :key="stat.key" 
        class="card animate-slide-up relative overflow-hidden group"
      ...
      >
        ...
        <div class="relative flex items-center justify-between">
          <div>
            ...
            <p class="text-3xl font-display font-bold text-gray-900 mt-2">
              {{ getStatValue(summary, stat.key) }}
            </p>
            <p v-if="stat.subLabel && summary" class="text-xs text-gray-400 mt-1">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="card animate-slide-up" style="animation-delay: 400ms">
        <div class="flex items-center justify-between mb-6">
          <h3 class="text-lg font-display font-bold text-gray-800 flex items-center gap-3">
          ...
          <span class="badge badge-primary">{{ summary?.schedules.today.length || 0 }}</span>
        </div>

        <div v-if="summary?.schedules.today.length" class="space-y-3">
          <div
            v-for="(schedule, index) in summary.schedules.today"
            :key="schedule.id"
            class="list-item bg-gradient-to-r from-gray-50 to-transparent border border-gray-100 animate-slide-in"
            :style="{ animationDelay: `${500 + index * 100}ms` }"
          >
            <div class="text-sm font-bold text-primary-600 bg-primary-50 px-3 py-1.5 rounded-lg">
              {{ formatTime(schedule.start_time) }}
            </div>
            <div class="flex-1 min-w-0">
              <div class="font-semibold text-gray-800 truncate">{{ schedule.title }}</div>
              <div v-if="schedule.location" class="text-sm text-gray-500 flex items-center gap-1 mt-0.5">
                <span>📍</span>
        ...
        <div v-else class="empty-state">
          ...
          <p class="text-gray-500">오늘 예정된 일정이 없어요</p>
          <p class="text-sm text-gray-400 mt-1">여유로운 하루 보내세요!</p>
        </div>
      </div>
      ...
      <div class="space-y-6">
        ...
        >
          <div class="flex items-center gap-4">
            ...
            <div>
              <h3 class="font-bold text-gray-800">오늘 마감</h3>
              <p class="text-red-600">
                <strong class="text-xl">{{ summary.todos.dueToday }}개</strong>의 할 일이 오늘 마감이에요!
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="card animate-slide-up" style="animation-delay: 500ms">
          <div class="flex items-center justify-between mb-6">
            <h3 class="text-lg font-display font-bold text-gray-800 flex items-center gap-3">
              <div class="w-10 h-10 bg-gradient-to-br from-violet-500 to-purple-500 rounded-xl flex items-center justify-center shadow-lg shadow-violet-500/20">
                <span class="text-lg">📊</span>
              </div>
              전체 현황
            </h3>
          ...
          <div class="space-y-4">
            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
              <div class="flex items-center gap-3">
                ...
                <span class="font-medium text-gray-700">총 메모</span>
              </div>
              <span class="text-xl font-bold text-gray-900">{{ summary?.memos.total || 0 }}</span>
            </div>
            
            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
              <div class="flex items-center gap-3">
                <div class="w-8 h-8 bg-emerald-100 rounded-lg flex items-center justify-center">✅</div>
                <span class="font-medium text-gray-700">완료된 할 일</span>
              </div>
              <span class="text-xl font-bold text-emerald-600">{{ summary?.todos.completed || 0 }}</span>
            </div>

            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
              <div class="flex items-center gap-3">
                <div class="w-8 h-8 bg-amber-100 rounded-lg flex items-center justify-center">⏳</div>
                <span class="font-medium text-gray-700">남은 할 일</span>
              </div>
              <span class="text-xl font-bold text-amber-600">{{ summary?.todos.incomplete || 0 }}</span>
            </div>

            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
              <div class="flex items-center gap-3">
                <div class="w-8 h-8 bg-violet-100 rounded-lg flex items-center justify-center">🔖</div>
                <span class="font-medium text-gray-700">저장된 북마크</span>
              </div>
              <span class="text-xl font-bold text-gray-900">{{ summary?.bookmarks.total || 0 }}</span>
            </div>
          </div>
        </div>
      </div>
