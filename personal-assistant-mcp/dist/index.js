...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            // 메모
            case 'create_memo': {
                const result = await memo.createMemo(args?.title, args?.content, args?.tags);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            case 'list_memos': {
                const result = await memo.listMemos(args?.query, args?.tag);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            case 'get_memo': {
                const result = await memo.getMemo(args?.id);
                if (!result) {
            ...
            case 'update_memo': {
                const result = await memo.updateMemo(args?.id, args?.title, args?.content, args?.tags);
                if (!result) {
                    return { content: [{ type: 'text', text: '메모를 찾을 수 없습니다.' }], isError: true };
                }
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            ...
            case 'delete_memo': {
                const success = await memo.deleteMemo(args?.id);
                return {
            ...
            // 할 일
            case 'create_todo': {
                const result = await todo.createTodo(args?.title, args?.description, args?.priority, args?.due_date, args?.tags);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            case 'list_todos': {
                const result = await todo.listTodos(args?.completed, args?.priority, args?.tag);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            case 'get_todo': {
                const result = await todo.getTodo(args?.id);
                if (!result) {
            ...
            case 'update_todo': {
                const result = await todo.updateTodo(args?.id, args?.title, args?.description, args?.completed, args?.priority, args?.due_date, args?.tags);
                if (!result) {
                    return { content: [{ type: 'text', text: '할 일을 찾을 수 없습니다.' }], isError: true };
                }
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            ...
            case 'delete_todo': {
                const success = await todo.deleteTodo(args?.id);
                return {
            ...
            case 'create_schedule': {
                const result = await schedule.createSchedule(args?.title, args?.start_time, args?.description, args?.end_time, args?.location, args?.tags);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            case 'list_schedules': {
                const result = await schedule.listSchedules(args?.date, args?.from_date, args?.to_date, args?.tag);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            case 'get_schedule': {
                const result = await schedule.getSchedule(args?.id);
                if (!result) {
            ...
            case 'update_schedule': {
                const result = await schedule.updateSchedule(args?.id, args?.title, args?.description, args?.start_time, args?.end_time, args?.location, args?.tags);
                if (!result) {
                    return { content: [{ type: 'text', text: '일정을 찾을 수 없습니다.' }], isError: true };
                }
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            ...
            case 'delete_schedule': {
                const success = await schedule.deleteSchedule(args?.id);
                return {
            ...
            case 'create_bookmark': {
                const result = await bookmark.createBookmark(args?.url, args?.title, args?.description, args?.tags);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            case 'list_bookmarks': {
                const result = await bookmark.listBookmarks(args?.query, args?.tag);
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            }
            case 'get_bookmark': {
                const result = await bookmark.getBookmark(args?.id);
                if (!result) {
            ...
            case 'update_bookmark': {
                const result = await bookmark.updateBookmark(args?.id, args?.url, args?.title, args?.description, args?.tags);
                if (!result) {
                    return { content: [{ type: 'text', text: '북마크를 찾을 수 없습니다.' }], isError: true };
                }
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
            ...
            case 'delete_bookmark': {
                const success = await bookmark.deleteBookmark(args?.id);
                return {
            ...
            case 'search_all': {
                ...
                const results = {
                    memos,
                    todos: todos.filter((t) => t.title.includes(searchQuery) || t.description.includes(searchQuery)),
                    schedules: schedules.filter((s) => s.title.includes(searchQuery) || s.description.includes(searchQuery)),
                    bookmarks,
                };
