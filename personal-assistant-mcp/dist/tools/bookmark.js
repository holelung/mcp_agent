...
// 북마크 생성
export async function createBookmark(url, title, description = '', tags = []) {
    const result = await queryInsert(`INSERT INTO bookmarks (url, title, description, tags)
     VALUES ($1, $2, $3, $4)
     RETURNING *`, [url, title, description, tags]);
    return result;
}
// 북마크 목록 조회
export async function listBookmarks(searchQuery, tag) {
...
export async function getBookmark(id) {
    return await queryOne('SELECT * FROM bookmarks WHERE id = $1', [id]);
}
// 북마크 수정
export async function updateBookmark(id, url, title, description, tags) {
    ...
    return await queryOne(`UPDATE bookmarks 
     SET url = $1, title = $2, description = $3, tags = $4
     WHERE id = $5
     RETURNING *`, [newUrl, newTitle, newDescription, newTags, id]);
}
// 북마크 삭제
...
export async function getBookmarkCount() {
    const result = await queryOne('SELECT COUNT(*) as count FROM bookmarks');
