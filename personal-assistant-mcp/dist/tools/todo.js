...
// 할 일 생성
export async function createTodo(title, description = '', priority = 'medium', due_date, tags = []) {
    const result = await queryInsert(`INSERT INTO todos (title, description, priority, due_date, tags)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING *`, [title, description, priority, due_date || null, tags]);
    return result;
}
// 할 일 목록 조회
export async function listTodos(completed, priority, tag) {
...
export async function getTodo(id) {
    return await queryOne('SELECT * FROM todos WHERE id = $1', [id]);
}
// 할 일 수정
export async function updateTodo(id, title, description, completed, priority, due_date, tags) {
    ...
    return await queryOne(`UPDATE todos 
     SET title = $1, description = $2, completed = $3, priority = $4, due_date = $5, tags = $6
     WHERE id = $7
     RETURNING *`, [newTitle, newDescription, newCompleted, newPriority, newDueDate, newTags, id]);
}
// 할 일 삭제
...
export async function getTodayTodos() {
    return await query('SELECT * FROM todos WHERE due_date = CURRENT_DATE AND completed = FALSE');
}
// 미완료 할 일 개수
export async function getIncompleteTodoCount() {
    const result = await queryOne('SELECT COUNT(*) as count FROM todos WHERE completed = FALSE');
